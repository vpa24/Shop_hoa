...
class M_thong_ke_doanh_thu extends database
{
    ...
    function theo_thang(){
      $sql="SELECT month(ngay_dat)as thang,sum(so_luong) as so_luong_ban FROM shop_hoa.chi_tiet_hoa_don inner join hoa_don on hoa_don.ma_hoa_don=chi_tiet_hoa_don.ma_hoa_don WHERE MONTH(ngay_dat) = MONTH(CURRENT_DATE())
