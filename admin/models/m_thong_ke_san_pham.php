...
{
    function theo_ngay(){
        $sql = "SELECT ngay_dat,sum(so_luong) as so_luong_ban FROM shop_hoa.chi_tiet_hoa_don inner join hoa_don on hoa_don.ma_hoa_don=chi_tiet_hoa_don.ma_hoa_don WHERE WEEK(ngay_dat)= WEEK(CURRENT_DATE())  and MONTH(ngay_dat) = MONTH(CURRENT_DATE())
